"""Four in a Row, by Jake Dobler jaecob.d1@gmail.com"""
...
def main():
    print("""Four in a Row, by Jake Dobler jaecob.d1@gmail.com
...
def getNewBoard():
    ...
    The keys are (ColumnIndex, rowIndex) tuples of two integers, and the values are one of the 'X', 'O', or '.' (empty space) strings."""
    board = {}
...
def displayBoard(board):
    ...
    '''Prepare a list to pass to the format() string method for the board template. The list holds all of the board's tiles (and empty spaces) going left to right, top to bottom:'''
    tileChars = []
    for rowIndex in range(BOARD_HEIGHT):
        for columnIndex in range(BOARD_WIDTH):
...
def isWinner(playerTile, board):
    """Returns True if `playerTitle` has four tiles in a row on `board`, otherwise returns False."""
    ...
    for columnIndex in range(BOARD_WIDTH):
        for rowIndex in range(BOARD_HEIGHT - 3):
            # Check for horizontal four-in-a-row going down:
            tile1 = board[(columnIndex, rowIndex)]
    ...
    for columnIndex in range(BOARD_WIDTH - 3):
        for rowIndex in range(BOARD_HEIGHT - 3):
            # Check for a four-in-a-row going down right diagonal:
            tile1 = board[(columnIndex, rowIndex)]
            ...
            # Check for a four-in-a-row going down left diagonal:
